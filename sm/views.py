...
def ssh_server(request):
	id=request.GET.get('id')
	return render_to_response('sm/ssh_server.html')

def execute_command(request):
	my_command = request.GET.get("my_command_input","pwd")
	print my_command
	if re.search("^shutdown",my_command):
		return HttpResponse("嘿，哥们儿，别这样干！")
	else:
	#     result=commands.getoutput(my_command)
	#     return HttpResponse("您好，这就是执行命令的结果: %s" % result)
	#return HttpResponse("execute_cmd.html")
		my_ssh=MySSH()
		my_ssh.login("192.168.3.205","root","ylgj@123",22)
		info=my_ssh.execute_command(my_command)
		info=json.dumps(info,encoding="utf8",ensure_ascii=False)
		return HttpResponse(info)

def execute_cmd(request):
	return render_to_response('sm/execute_cmd.html')

def create_server_config(request):
	info={"status":False,"content":"暂无消息"}
	info=json.dumps(info,encoding="utf8",ensure_ascii=False)
	return HttpResponse(info)